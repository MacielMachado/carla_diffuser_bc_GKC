...
if __name__ == '__main__':
    ...
    alpha_list = ['fixed_0-9', 'fixed_1-0', 'cosine', 'fixed_0-0', ]
    batch_size_list = [1024, 512, 32]
    embedding_dim_list = [32, 64, 128]
    ...
    architecture = ['distribution_diffusion', 'mse_diffusion']
    ...
    for index, item in enumerate(params_list):
        ...
        try:
            learn_bc(policy, device, gail_train_loader, gail_val_loader,
                    ...
                    config=config,
                    name=f'distribution-diffusion-search-hyperparameters-t/version_{index}',)
            env.close()
            del env
